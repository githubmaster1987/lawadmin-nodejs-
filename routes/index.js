...
exports.signup = function (req, res) {
	...
	db.exec(sql, [req.body.username], function(err, results) {
		...
		if(results.length) { // If data found then send notify existing user
			res.status(409).send({success:false, message: "Existing user."});
			// res.send({success:true, data:results});
		} else {
			// If no data then add a user 
			...
			db.exec(sql, req.body, function(err, results) {
				...
				if(results.affectedRows === 1) {
					...
					res.status(408).send({success:false, message: "Error while adding a user."});
				}
			});
		}
	});
